...
void inserirInicio ( NO **lista, float elem){
    ...
    if (novo != NULL){
        ...
        if (!listaVazia(*lista)) (*lista)->esq = novo;
...
/*
void removerValor( NO **lista, float elem ){
    
    NO *q;
    if ( ( q= pesquisarValor( *lista, elem ) ) != NULL ){
        
        if( *lista == q ) removerInicio( lista );
        else{

            ( q->esq )->dir = q->dir;

            if ( q->dir != NULL ) ( q->dir )->esq = q->esq;
